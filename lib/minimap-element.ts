...
class MinimapElement {
   ...
   * @access private
   */
  measureHeightAndWidth(visibilityChanged: boolean, forceUpdate = true) {
    if (!this.minimap) {
      return
    }
 ...
 * @access private
 */
function makeTranslate(x = 0, y = 0, useHardwareAcceleration = false): string {
 ...
 * @access private
 */
function makeScale(x = 0, y = x, useHardwareAcceleration = false): string {
  if (useHardwareAcceleration) {
